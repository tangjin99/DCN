	...
	if (idx >= 0) {
		...
		if (qIndex != 0) { //not highest priority
			...
			if (m_mmu->CheckIngressAdmission(inDev, qIndex, p->GetSize(), found) && m_mmu->CheckEgressAdmission(idx, qIndex, p->GetSize(), found)) {			// Admission control
				...
				m_mmu->UpdateIngressWaitingtime(inDev, idx, qIndex, DynamicCast<QbbNetDevice>(m_devices[idx])->GetDataRate().GetBitRate());
...
void SwitchNode::SwitchNotifyDequeue(uint32_t ifIndex, uint32_t qIndex, Ptr<Packet> p) {
	...
	if (qIndex != 0) {
		...
		m_mmu->RemoveFromIngressWaitingtime(inDev, ifIndex, qIndex, DynamicCast<QbbNetDevice>(m_devices[ifIndex])->GetDataRate().GetBitRate());
